  ...
  /******/ var __webpack_modules__ = [
      ...
      async function generateVSProjectFiles() {
        ...
        try {
          ...
          if (error instanceof Error) {
            ...
            if (!error.message.includes("EPERM")) {
              vscode.window.showErrorMessage(
                `Error copying main files: ${error}`,
              );
              return;
            } else {
              return;
    ...
    /***/ function (__unused_webpack_module, exports, __webpack_require__) {
      ...
      function copyTemplateFiles(projectFolder, projectDir, projectName) {
        ...
        fs.readdirSync(templateDir).forEach((file) => {
          if (file !== "template.sln" && file.startsWith("template.")) {
            fs.copyFileSync(
              path_1.default.join(templateDir, file),
              path_1.default.join(projectFolder, file),
            );
          }
        });
        ...
        fs.copyFileSync(
          path_1.default.join(templateDir, slnFile),
          path_1.default.join(projectDir, slnFile),
        );
        ...
        fs.readdirSync(projectFolder).forEach((file) => {
          if (file.startsWith("template.")) {
            ...
            fs.renameSync(
              path_1.default.join(projectFolder, file),
              path_1.default.join(projectFolder, newFileName),
            );
          }
        });
      ...
      function copyFiles(workingDir, projectFolder) {
        ...
        fs.readdirSync(workingDir).forEach((file) => {
          ...
          if (extensions.includes(ext)) {
            fs.copyFileSync(
              path_1.default.join(workingDir, file),
              path_1.default.join(projectFolder, file),
            );
          }
        });
      ...
      function replaceIdsInSolutionFile(projectName, projectDir) {
        const solutionFilePath = path_1.default.join(
          projectDir,
          `${projectName}.sln`,
        );
        ...
        content = content.replace(
          /PROJECTID/g,
          (0, guidGenerator_1.generateGUID)(),
        );
        content = content.replace(
          /SOLUTIONID/g,
          (0, guidGenerator_1.generateGUID)(),
    ...
    /***/ function (__unused_webpack_module, exports, __webpack_require__) {
      ...
      function appendFileTypesToFilters(projectFolder, projectName) {
        ...
        fs.appendFileSync(
          path_1.default.join(projectFolder, `${projectName}.vcxproj.filters`),
          filters,
        );
        fs.appendFileSync(
          path_1.default.join(projectFolder, `${projectName}.vcxproj`),
          vcxproj,
        );
      }
